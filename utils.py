...
def slice_tensor_tensor(tensor, tensor_slice, cuda):
    tensor_ = tensor.cpu().data.numpy()
    tensor_slice_ = tensor_slice.cpu().data.numpy()
    output = tensor_[np.asarray(range(tensor_slice.size(0))), tensor_slice_]
    output = torch.from_numpy(output)
    return output


def plot(data={}, loc="viz.png", x_label="", y_label="", title="", kind='line',
         legend=True, index_col=None, clip=None, moving_average=False):
    if all([len(data[key]) > 1 for key in data]):
        if moving_average:
            smoothed_data = {}
            for key in data:
                smooth_scores = [np.mean(data[key][max(0, i - 10):i + 1])
...
def plot_and_write(plot_dict, loc, x_label="", y_label="", title="",
                   kind='line', legend=True, moving_average=False):
    for key in plot_dict:
        plot(data={key: plot_dict[key]}, loc=loc + ".png", x_label=x_label,
             y_label=y_label, title=title, kind=kind, legend=legend,
             index_col=None, moving_average=moving_average)
...
class ExperienceReplay(object):
    ...
    """
    def __init__(self, max_size=100, history_len=1, state_shape=None,
                 action_dim=1, reward_dim=1, state_dtype=np.uint8, rng=None):
        if rng is None:
        ...
        self.states = np.zeros([self.max_size] + list(self.state_shape),
